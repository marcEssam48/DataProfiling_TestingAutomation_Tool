...
def get_core_tbl_hist_keys_list(Core_tables, tbl_name, history_column_list):
    try:
        ...
        return hist_keys_list
    return hist_keys_list
def get_data_type_for_history(core_tables,column_name):
    data_type = core_tables[(core_tables["Column name"].str.strip() == column_name.strip())]
    try:
        current_data_type = data_type["Data type"].item()
    except:
        data_type = "Undefined"
