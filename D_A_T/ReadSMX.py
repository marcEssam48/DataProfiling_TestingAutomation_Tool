...
class ReadSmx:
    ...
    def build_source_scripts(self, smx_file_path, output_path, source_output_path, source_name, Loading_Type):
        ...
        BMAP_values = delayed(funcs.get_sheet_data)(pm.parquet_db_name, smx_file_path, output_path, pm.BMAP_values_sht)
        BMAP = delayed(funcs.get_sheet_data)(pm.parquet_db_name, smx_file_path, output_path, pm.BMAP_sht)
        BKEY = delayed(funcs.get_sheet_data)(pm.parquet_db_name, smx_file_path, output_path, pm.BKEY_sht)
        Core_tables = delayed(funcs.get_sheet_data)(pm.parquet_db_name, smx_file_path, output_path, pm.Core_tables_sht)

        source_name_filter = [['Source', [source_name]]]
        stg_source_name_filter = [['Source system name', [source_name]]]

        Table_mapping = delayed(funcs.get_sheet_data)(pm.parquet_db_name, smx_file_path, output_path, pm.Table_mapping_sht, source_name_filter)
        STG_tables = delayed(funcs.get_sheet_data)(pm.parquet_db_name, smx_file_path, output_path, pm.STG_tables_sht, stg_source_name_filter)
