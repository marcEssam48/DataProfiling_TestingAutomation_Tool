...
from D_A_T.UDI.read_smx_sheet.app_Lib import manage_directories as md, functions as funcs
from dask import compute, delayed, config
from dask.diagnostics import ProgressBar
from D_A_T.UDI.read_smx_sheet.templates import D110, D300, D320, D200, D330, D400, D610, D640, D609
from D_A_T.UDI.read_smx_sheet.templates import testing_script_01, testing_script_02
from D_A_T.UDI.read_smx_sheet.templates import PROCESS_CHECK_TEST_SHEET, CSO_TEST_SHEET, NULLS_TEST_SHEET, DUP_TEST_SHEET
from D_A_T.UDI.read_smx_sheet.templates import BMAP_DUP_CD_TEST_SHEET,BMAP_DUP_DESC_TEST_SHEET,BMAP_NULL_TEST_SHEET
from D_A_T.UDI.read_smx_sheet.templates import DATA_SRC_TEST_SHEET, BMAP_CHECK_TEST_SHEET , BMAP_UNMATCHED_TEST_SHEET
from D_A_T.UDI.read_smx_sheet.templates import HIST_STRT_END_NULL_TEST_SHEET, HIST_DUP_TEST_SHEET ,HIST_STRT_GRT_END_TEST_SHEET,HIST_TIME_GAP_TEST_SHEET
from D_A_T.UDI.read_smx_sheet.templates import HIST_STRT_NULL_TEST_SHEET, RI_TEST_SHEET, compare_testing_inputview
from D_A_T.UDI.read_smx_sheet.templates import D410, D415, D003, D630, D420, D210, D608, D615, D000, gcfr, D620, D001, D600, D607, D002, D340,stgCounts,stg_accepted_count,dataValidation,D215
from D_A_T.UDI.read_smx_sheet.templates import dataValidation,stgCounts
...
class ConfigFile:
    def __init__(self, config_file=None, config_file_values=None ):
        ...
        self.ip = self.config_file_values["ip"]
        self.username = self.config_file_values["username"]
        self.password = self.config_file_values["password"]
        self.scripts_status = self.config_file_values["scripts_status"]
        ...
        self.core_table_name = self.config_file_values["core_table_name"]
...
class GenerateScripts:
    def __init__(self, config_file=None, config_file_values=None ):
        ...
        self.scripts_flag = config_file_values["scripts_flag"]
        self.testing_categories = str(config_file_values["testing_catgs"]).replace("[","").replace("]","").replace("'","").replace(" ","").split(",")
        self.core_table_name = config_file_values["core_table_name"]
        self.scripts_status = str(config_file_values["scripts_status"]).lower()
        # print(self.scripts_status)

        # print(self.ip)
        # print(self.username)
        # print(self.password)
        # print(self.testing_categories)

        # self.connect = teradatasql.connect(host="172.19.3.150", user="Data_profiling_Tool", password="P@ssw0rd",
        #                               encryptdata=True)

        try:
            self.script_connect_run = teradatasql.connect(host=str(self.cf.ip), user=str(self.cf.username), password=str(self.cf.password),
                                                     encryptdata=True)

            self.connect = teradatasql.connect(host=str(self.cf.ip), user=str(self.cf.username), password=str(self.cf.password),
                                                     encryptdata=True)
    ...
    def generate_scripts(self):
                    ...
                    for system_index, system_row in teradata_sources.iterrows():
                        try:
                            ...
                            if Loading_Type != "":
                                ...
                                core_table_filter = [['Target table name', [self.core_table_name]]]
                                # print(core_table_filter)

                                if source_name == "Null":
                                    all_table_mapping = delayed(funcs.read_excel)(smx_file_path, self.Table_mapping_sht, core_layer_filter)
                                    Table_mapping = delayed(funcs.read_excel)(smx_file_path, self.Table_mapping_sht, core_table_filter)
                                ...
                                source_output_path = os.path.join(main_output_path, "")
                                output_path_testing = ""
                                ...
                                if self.scripts_status == "generated" or self.scripts_status == "all":
                                ...
                                if self.scripts_flag == 'All' or self.scripts_flag == 'Testing':
                                    ...
                                    #     print(self.scripts_status)
                                        ...
                                        self.parallel_create_output_source_path.append(delayed(md.create_folder)(compare_stg_counts_output_path_testing))

                                    for test_case in self.testing_categories:
                                        # print(test_case)
                                        if test_case == "BMAPS":
                                            if source_name == "Null":
                                                self.parallel_templates.append(delayed(BMAP_CHECK_TEST_SHEET.bmap_check)(self.cf,bmaps_output_path_testing,all_table_mapping, Core_tables, BMAP_values,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(BMAP_DUP_CD_TEST_SHEET.bmap_dup_check)(self.cf, bmaps_output_path_testing,all_table_mapping,Core_tables,BMAP_values,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(BMAP_DUP_DESC_TEST_SHEET.bmap_dup_desc_check)(self.cf, bmaps_output_path_testing, all_table_mapping,Core_tables, BMAP_values,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(BMAP_NULL_TEST_SHEET.bmap_null_check)(self.cf,bmaps_output_path_testing, all_table_mapping, Core_tables, BMAP_values,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(BMAP_UNMATCHED_TEST_SHEET.bmap_unmatched_values_check)(self.cf,bmaps_output_path_testing,all_table_mapping, Core_tables, BMAP,BMAP_values,self.connect, self.script_connect_run))

                                            else:
                                                self.parallel_templates.append(delayed(BMAP_CHECK_TEST_SHEET.bmap_check)(self.cf,bmaps_output_path_testing, core_Table_mapping,Core_tables, BMAP_values,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(BMAP_DUP_CD_TEST_SHEET.bmap_dup_check)(self.cf, bmaps_output_path_testing,core_Table_mapping,Core_tables,BMAP_values,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(BMAP_DUP_DESC_TEST_SHEET.bmap_dup_desc_check)(self.cf, bmaps_output_path_testing, core_Table_mapping,Core_tables, BMAP_values,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(BMAP_NULL_TEST_SHEET.bmap_null_check)(self.cf,bmaps_output_path_testing, core_Table_mapping, Core_tables, BMAP_values,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(BMAP_UNMATCHED_TEST_SHEET.bmap_unmatched_values_check)(self.cf,bmaps_output_path_testing,core_Table_mapping, Core_tables, BMAP,BMAP_values,self.connect, self.script_connect_run))

                                        if test_case == "History_Handling" :
                                            try:
                                                self.parallel_templates.append(delayed(HIST_STRT_END_NULL_TEST_SHEET.hist_start_end_null_check)(self.cf, history_output_path_testing, core_Table_mapping,Core_tables,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(HIST_DUP_TEST_SHEET.hist_dup_check)(self.cf,history_output_path_testing,core_Table_mapping,Core_tables,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(HIST_STRT_GRT_END_TEST_SHEET.hist_start_end_null_check)(self.cf,history_output_path_testing,core_Table_mapping, Core_tables,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(HIST_TIME_GAP_TEST_SHEET.hist_timegap_check)(self.cf,history_output_path_testing, core_Table_mapping, Core_tables,self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(HIST_STRT_NULL_TEST_SHEET.hist_start_null_check)(self.cf, history_output_path_testing,core_Table_mapping,Core_tables,self.connect, self.script_connect_run))
                                            except Exception as e :
                                                print(e)
                                        if test_case == "RI":
                                            try:
                                                self.parallel_templates.append(delayed(RI_TEST_SHEET.ri_check)(self.cf, ri_output_path_testing,core_Table_mapping, RI_relations,self.connect, self.script_connect_run))
                                            except Exception as e:
                                                print(e)

                                        if test_case == "Data_src":
                                            self.parallel_templates.append(delayed(DATA_SRC_TEST_SHEET.data_src_check)(self.cf,data_src_output_path_testing,source_name, core_Table_mapping,Column_mapping,self.connect, self.script_connect_run))
                                        if test_case == "Duplicates":
                                            self.parallel_templates.append(delayed(DUP_TEST_SHEET.duplicates_check)(self.cf,duplicate_output_path_testing,core_Table_mapping,Core_tables,self.connect, self.script_connect_run))

                                        if test_case == "Nulls":
                                            self.parallel_templates.append(delayed(NULLS_TEST_SHEET.nulls_check)(self.cf,nulls_output_path_testing,core_Table_mapping, Core_tables , Column_mapping,self.connect, self.script_connect_run))

                                        if test_case == "CSO":
                                            self.parallel_templates.append(delayed(CSO_TEST_SHEET.cso_check)(self.cf, cso_output_path_testing,source_name, core_Table_mapping,Column_mapping,self.connect, self.script_connect_run))

                                        if test_case == "Compare_Tests":
                                            try:
                                                # md.create_folder(source_output_path)
                                                # D620.d620(self.cf, input_view_output_path_testing, core_Table_mapping, Column_mapping, Core_tables, Loading_Type,'TESTING')
                                                # compare_testing_inputview.compare_views_check(self.cf, input_view_output_path_testing, core_Table_mapping,'FROM_TESTING_TO_UDI')
                                                ...
                                                self.parallel_templates.append(delayed(D620.d620)(self.cf, input_view_output_path_testing, core_Table_mapping, Column_mapping, Core_tables, Loading_Type,'TESTING' , input_view_output_path_testing , core_Table_mapping,self.connect, self.script_connect_run))

                                            except Exception as e:
                                                print(e)
                                        if test_case == "STG_CHECKS":
                                            try:
                                                self.parallel_templates.append(delayed(stgCounts.stgCounts)(self.cf,compare_stg_counts_output_path_testing,system_row,STG_tables,Loading_Type,'Accepted',self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(stgCounts.stgCounts)(self.cf,compare_stg_counts_output_path_testing,system_row,STG_tables,Loading_Type,'All',self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(dataValidation.dataValidation)(self.cf,compare_stg_counts_output_path_testing,source_name,system_row,STG_tables,Loading_Type,'Accepted',self.connect, self.script_connect_run))
                                                self.parallel_templates.append(delayed(dataValidation.dataValidation)(self.cf, compare_stg_counts_output_path_testing,source_name,system_row, STG_tables,Loading_Type,'All',self.connect, self.script_connect_run))
                                            except Exception as e:
                                                print(e)
                                    ...
                                    # self.parallel_templates.append(delayed(BMAP_CHECK_TEST_SHEET.bmap_check)(self.cf, bmaps_output_path_testing, core_Table_mapping, Core_tables,BMAP_values))
                                    # self.parallel_templates.append(delayed(BMAP_DUP_CD_TEST_SHEET.bmap_dup_check)(self.cf, bmaps_output_path_testing, core_Table_mapping, Core_tables,BMAP_values))
                                    # self.parallel_templates.append(delayed(BMAP_DUP_DESC_TEST_SHEET.bmap_dup_desc_check)(self.cf, bmaps_output_path_testing,core_Table_mapping,Core_tables,BMAP_values))
                                    ...
                                    # self.parallel_templates.append(delayed(BMAP_UNMATCHED_TEST_SHEET.bmap_unmatched_values_check)(self.cf, bmaps_output_path_testing,core_Table_mapping,Core_tables,BMAP,BMAP_values))
        ...
        if len(self.parallel_templates) > 0:
            ...
            scheduler_value = 'threads' if self.cf.read_sheets_parallel == 1 else ''
            with config.set(scheduler=scheduler_value, pool=ThreadPool(20)):
