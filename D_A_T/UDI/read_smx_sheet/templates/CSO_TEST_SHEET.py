...
@Logging_decorator
def cso_check(cf, source_output_path, source_name, table_mapping, Column_mapping,connect, script_connect_run):
    ...
    for table_Mapping_index,table_Mapping_row in table_mapping.iterrows():
        for table_maping_index, column_mapping_row in Column_mapping_NID.iterrows():
            if table_Mapping_row['Mapping name'] == column_mapping_row['Mapping name']:
                ...
                f.write(call_exp)
                count = count + 1
                CHILD_TABLE = "BKEY_1_PRTY , CSO_PERSON"
                DB_PARAM = cf.base_DB + " , " + cf.UTLFW_v + " , " + " STG_ONLINE "


                script_to_be_sent = call_line1 + call_line2 + call_line3 + call_line4 + call_line5 + call_line6 +call_line7.replace("\n", "")
                # print(script_to_be_sent)
                test_logger.insert_testing_logs(script_to_be_sent, target_table_name,
                                                CHILD_TABLE, "CSO TEST", process_name_line, source_name,
                                                "CSO_TEST",
